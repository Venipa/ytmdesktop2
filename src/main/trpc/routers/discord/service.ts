...
export default class DiscordProvider extends BaseProvider implements AfterInit, OnDestroy {
	...
	async updateTrackProgress(isPlaying: boolean, mediaProgress: number = 0, updateImmediate: boolean = false) {
		if (trackService.trackData && this.isConnected) {
			if (updateImmediate) this._updateActivity(discordEmbedFromTrack(trackService.trackData, isPlaying, mediaProgress));
			else this.updateActivity(discordEmbedFromTrack(trackService.trackData, isPlaying, mediaProgress));
		}
	}
