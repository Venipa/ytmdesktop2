...
import { BrowserWindow, BrowserWindowConstructorOptions, IpcMainEvent, WebContentsView, app, protocol } from "electron";
...
const runApp = async function () {
	...
	async function createRootWindow(options?: BrowserWindowConstructorOptions) {
		...
		serverMain.on(
			"app.loadStart",
			debounce(() => {
				if (!win.contentView.children?.find((x) => loadingView.webContents && x === loadingView)) win.contentView.addChildView(loadingView);
			}, 100),
		);
		...
		youtubeView.webContents.on("page-title-updated", (ev, title) => youtubeView.webContents.emit("window-title-updated", title));
		const __data = {
			main: win,
			views: {
				youtubeView,
	...
	app.on("ready", async () => {
		...
		if (startupService.isStartupContext ? !startupService.isEnabled || !startupService.isInitialMinimized : !startupService.isMinimizedArg) mainWindow.main.show();
		serviceCollection.providers.forEach((p) => p.__registerWindows(mainWindow));
		serviceCollection.exec("AfterInit");
	});
