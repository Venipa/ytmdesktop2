...
export default class UpdateProvider extends BaseProvider implements BeforeStart, AfterInit {
	...
	AfterInit() {
		...
		if (this.settingsInstance.instance.app.autoupdate && !isDevelopment) this.onCheckUpdate().catch((err) => this.logger.error("Error checking for update", err));
	...
	private async _checkUpdate() {
		...
		return await autoUpdater.checkForUpdates().then((x) => {
			if (!x?.updateInfo || !this.isUpdateInRange(x.updateInfo.version)) throw new Error("No Update available");
			return x;
		});
	}
	...
	onCheckUpdate() {
		return this._checkUpdate()
			.then((x) => {
				const releaseNotes = (typeof x.updateInfo.releaseNotes === "string" ? x.updateInfo.releaseNotes : x.updateInfo.releaseNotes?.map((x) => x.note).join("\n"))
					?.replace(/<[^>]+>/g, "")
					.trimStart();
				return dialog
