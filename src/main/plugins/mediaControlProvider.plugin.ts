...
export default class MediaControlProvider extends BaseProvider implements AfterInit, BeforeStart, OnDestroy {
	...
	async AfterInit() {
		...
		if (!this.mediaProviderEnabled()) this.xosmsLog.warn("XOSMS is disabled", ":: Status:", `Provider: ${!!this._mediaProvider}, Enabled: ${this.mediaProviderEnabled()}`);
	}
	get instance() {
	...
	async handleTrackMediaOSControlChange(trackData: TrackData) {
		...
		try {
			...
			this._mediaProvider.artist = trackData.video.author;
			if (albumThumbnail) this._mediaProvider.setThumbnail(await MediaPlayerThumbnail.create(MediaPlayerThumbnailType.Uri, albumThumbnail));
