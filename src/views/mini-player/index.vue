<template>
  <div class="h-full absolute inset-0 overflow-hidden bg-black flex flex-col"
       :style="{
        ...(accentColor && showWinBorder === 'win11' && (state?.maximized ? { borderRadius: '0px' } : { borderRadius: '8px' }) || {}),
        ...(accentColor && showWinBorder && (showWinBorder === 'win11' ? { borderWidth: '2.5px', borderStyle: 'solid' } : showWinBorder ? { borderWidth: '1px', borderStyle: 'solid' } : {}) || {}),
    ...
    <div class="absolute inset-0">
      <div class="absolute inset-0 opacity-[.25]"
           ...
           v-if="thumbnail && accentColor"></div>
      <div class="absolute inset-0 bg-no-repeat bg-cover bg-center opacity-[.25] scale-125 blur-[8px]"
           v-if="thumbnail"
    ...
    <div class="flex flex-col flex-1">
        ...
        <div class="flex items-start space-x-6">
          <div class="track-thumbnail flex flex-shrink-0 items-center shadow justify-center relative">
            ...
            <template v-if="thumbnail">
              <div class="absolute inset-0 rounded-[inherit] z-[1]"
                   v-if="accentColor"
                   :style="{
                    boxShadow: `10px 12px 12px -2px ${accentColor}50, 0 0 0 .1rem ${accentColor}`,
                  }"></div>
              <div class="absolute -inset-2 rounded-xl z-[2]"
                   v-if="accentColor"
                   :style="{
          ...
          <div class="flex flex-col flex-1 h-full truncate">
            <div class="min-w-0 flex-auto space-y-1 font-semibold truncate"
                 v-if="track?.video">
              ...
              <div class="text-zinc-400 text-sm space-x-1 flex items-center whitespace-pre"
                   v-if="time">
      ...
      <div class="flex flex-col relative z-10">
        <div class="group pt-4 -mt-4"
             v-if="time">
          <div class="h-1 group-hover:h-2 bg-white transition-all ease-in-out duration-150"
               :style="{
                width: `${time[2]}%`,
                maxWidth: '100%',
                ...(accentColor ? { backgroundColor: accentColor } : {}),
              }"
               ref="progressHandle"></div>
        </div>
...
import { refIpc, refWindowState } from "@/utils/Ipc";
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      Promise.all([
        ...
        showWinBorder.value = window.process.platform === "win32" ? (isWin11 ? "win11" : true) : false;
        isTop.value = stayTop;
        console.log({ showWinBorder: showWinBorder.value, platform: window.process.platform, isWin11 })
      });
    });
