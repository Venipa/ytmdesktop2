...
import { fromMainEvent } from "@main/ipc/rxjs";
...
export function syncWindowStateToWebContents(win: BrowserWindow) {
	...
	return (view: WebContents) => {
		...
		const handleStates = () => {
			...
			if (!state) {
				serverMain.emit("windowState", state);
			} else {
				...
				serverMain.emit("windowState", next);
			}
		};
		...
		subs.push(
			...
			...["will-resize", "will-move"].map((d) => fromMainEvent(win, d).pipe(debounceTime(50)).subscribe(handleStates)),
		...
		if (platform.isMacOS) {
			subs.push(...["enter-full-screen", "leave-full-screen"].map((d) => fromMainEvent(win, d).pipe(delay(50)).subscribe(handleStates)));
		}
		win.once("close", () => {
...
>(ctx: T1) {
	...
	subs.push(
		...
		...["will-resize", "will-move"].map((d) => fromMainEvent(ctx.main, d).pipe(debounceTime(50)).subscribe(handleStates)),
		fromMainEvent(ctx.main, "hide").subscribe(() => (hidden = true)),
		fromMainEvent(ctx.views.youtubeView.webContents, "did-navigate-in-page").subscribe(handleStates),
	...
	if (platform.isMacOS) {
		subs.push(...["enter-full-screen", "leave-full-screen"].map((d) => fromMainEvent(ctx.main, d).pipe(delay(50)).subscribe(handleStates)));
	}
	ctx.main.once("close", () => {
		Object.entries(handles).forEach(([k, h]) => {
