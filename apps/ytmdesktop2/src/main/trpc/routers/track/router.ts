...
export const trackRouter = router({
	...
	like: publicProcedure.input(z.object({ liked: z.boolean() })).mutation(({ input }) => trackService.postTrackLike(undefined, input.liked)),
	dislike: publicProcedure.input(z.object({ disliked: z.boolean() })).mutation(({ input }) => trackService.postTrackDisLike(undefined, input.disliked)),
